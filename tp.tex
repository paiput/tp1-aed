...
\fbox{
    \begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule}
        ...
        \vspace{0.3em}
        % ----- agregarBloque ----------------------------
        \begin{proc}{agregarBloque}{\Inout berretacoin: Berretacoin}{}
            \requiere{berretacoin = B_{0}}
            \asegura{berretacoin.usuarios = B_{0}.usuarios}
            \asegura{(|B_{0}.bloques| \le 3000) \implicaLuego (\existe[unalinea]{i}{\ent}{(0 \le i < |B_{0}.usuarios|) \implicaLuego (berretacoin.usuarios[i].monedas = B_{0}.usuarios[i].monedas + 1)\yLuego (berretacoin.bloques = B_{0}.bloques \text{ ++ }\langle |B_{0}.bloques| + 1,\langle\rangle \text{ ++ } \langle 1, 0, B_{0}.usuarios[i].id, 1 \rangle\rangle)})}
            \asegura{\text{falta completar la version si ya pasaron los primeros 3000 bloques, la parte sencilla}}
        \end{proc} \} \par
        \vspace{0.5em}
