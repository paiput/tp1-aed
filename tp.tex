...
Un bloque es una secuencia de \textit{transaccion}, esta ultima tiene un limite de hasta 50 posibles transacciones, y por lo tanto \textit{bloques} no es mas que una \TLista{\TLista{transaccion}}, y de esta manera los subindices de \textit{bloques} representan el \textit{id} de cada \textit{bloque}, que por enunciado sabemos que son enteros positivos secuenciales.
...
\fbox{
    \begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule}
        ...
        \vspace{0.2cm}
        TAD Berretacoin \{ \\
        \qquad obs bloques: \TLista{\TLista{transaccion}} \\
        ...
        \vspace{0.3em}
        % ----- agregarBloque ----------------------------
        \begin{proc}{agregarBloque}{\Inout berretacoin: Berretacoin, \In t: \TLista{transaccion}}{\ent}
            \requiere{berretacoin = B_{0}}
            \requiere{|t| \le 50}
            \asegura{berretacoin.usuarios = B_{0}.usuarios}
            \asegura{(|B_{0}.bloques| \le 3000) \implicaLuego (\existe[unalinea]{i}{\ent}{(0 \le i < |B_{0}.usuarios|) \implicaLuego ((berretacoin.usuarios[i].monedas = B_{0}.usuarios[i].monedas + 1)\yLuego (berretacoin.bloques = B_{0}.bloques \text{ ++ } (t \text{ ++ } \langle \textit{id\_transaccion: } 1,\ \textit{id\_comprador: } 0,\ \textit{id\_vendedor: } B_{0}.usuarios[i].id,\ \textit{monto: } 1 \rangle)))})}
            \asegura{(|B_{0}.bloques| > 3000) \implicaLuego (berretacoin.bloques = B_{0}.bloques \text{ ++ } t)}
        \end{proc} \} \par
        \vspace{0.5em}
