...
% ------------------------- TAD -------------------------- %
    ...
    \begin{tcolorbox}[
        ...
        \begin{proc}{agregarBloque}{\Inout b: Berretacoin, \In transacciones: \TLista{transaccion}}{}
            \text{/* Cantidad limite de transacciones = 50 */} \\
            \requiere{b = B_{0}}
            \text{/* Los ids de las transacciones son consecutivos arrancando desde el 0 o no tenga transacciones */} \\
            ...
            \text{/* Si en transacciones aparece un comprador que en }B_0\text{ no está como usuario, el mismo debe aparecer} \\ \text{como vendedor primero */} \\
            \requiere{
            ...
            \text{/* La suma de los montos recibidos y gastados es }\geq 0\text{ sin importar si el comprador/vendedor estaba } \\ \text{ya en la lista de usuarios o no */} \\
            ...
            \text{/* El vendedor es algún usuario arbitrario siempre distinto para todas las operaciones de creacion} \\ \text{de berretacoin */} \\
            \requiere{
            ...
            \text{/* El bloque nuevo se agrega a la lista de bloques de berretacoin */} \\
            \asegura{b.bloques = B_0 ++ \langle id_bloque:|B_0.bloques|, transacciones:transacciones\rangle}
            \text{/* Todos los vendedores que están en transacciones y no están en } B_0.usuarios \text{, están en } b.usuarios \text{ */} \\
        ...
        \aux{montoTotalOperadoBloques}{bloques: \TLista{bloque}}{\ent}{
            \\ \qquad\quad \left(\sum_{k=0}^{|bloques| - 1} montoTotalOperado(transaccionesDesde(ifThenElse(b.bloques[k].id_bloque \leq 3000, 1, 0),
                \\ \qquad\qquad\qquad b.bloques[k].transacciones))\Big)}
        \vspace{0.5em}

        \aux{cantTotalDeOperacionesBloques}{bloques: \TLista{bloque}}{\ent}{
            \\ \qquad\quad \left(\sum_{k=0}^{|bloques| - 1} |b.bloques[k].transacciones| - ifThenElse(b.bloques[k].id_bloque \leq 3000, 1, 0)\right)}
        \vspace{0.5em}
