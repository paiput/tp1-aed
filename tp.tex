    ...
    \begin{tcolorbox}[
        ...
        \begin{proc}{agregarBloque}{\Inout b: Berretacoin, \In transacciones: \TLista{transaccion}}{}
            ...
            \requiere{
                ...
                \textbf{\textcolor{brown}{(}}transacciones[i].id\_comprador = transacciones[j].id\_vendedor\textbf{\textcolor{brown}{)}}}\textbf{\textcolor{red}{)}}\textbf{\textcolor{blue}{)}}}
            }
            ...
            \textbf{\textcolor{blue}{(}}\paraTodo[unalinea]{j}{\ent}{
                ((0 \le j < |transacciones|) \land (b.usuarios[i].id = transacciones[j].id\_comprador)) \implicaLuego
                \textbf{\textcolor{red}{(}}
                b.usuarios[i].monto + montoRecibido(transacciones,\ usuarios[i].id,\ j) - montoGastado(transacciones,\ usuarios[i].id,\ j)) \ge 0\textbf{\textcolor{red}{)}}}\textbf{\textcolor{blue}{)}}}
            }
            \textcolor{babyblue}{/* Verifica que ningún usuario nuevo gastó más de lo que adquirió */} \\
            \requiere{\paraTodo[unalinea]{i}{\ent}{((0 \le i < |transacciones|) \land_L \\ (\lnot perteneceUsuario(b.usuarios, transacciones[i].id\_comprador))) \implicaLuego (montoRecibido(transacciones,\ transacciones[i].id\_comprador,\ i) - montoGastado(transacciones,\ transacciones[i].id\_comprador,\ i))}}
            ...
            \requiere{
                (|b.bloques| \le 3000) \implicaLuego \textbf{\textcolor{blue}{(}} \\
                \setlength{\leftskip}{8em}
                $(|transacciones| > 0)\ \land_L \\
                (transacciones[0].id\_comprador = 0)\ \land_L \\
                (transacciones[0].monto = 1)\ \land_L \\
                (noGanoMonedaGratis(b.bloques, transacciones[0].id\_vendedor))\ \land_L \\
                (transacciones[0].id\_vendedor $>$ 0) \land_L \\
                (\paraTodo[unalinea]{i}{\ent}{(1 \le i < |transacciones|) \implicaLuego ((transacciones[i].id\_comprador \neq 0) \land_L$
                    \\ \qquad (transacciones[i].id\_vendedor \neq 0)))}\\
                \textbf{\textcolor{blue}{)}}
                \\
            ...
            \requiere{(|b.bloques| > 3000) \implicaLuego (\paraTodo[unalinea]{i}{\ent}{(0 \le i < |transacciones|) \implicaLuego ((transacciones[i].id\_comprador \neq 0) \land_L (transacciones[i].id\_vendedor \neq 0))})}
            ...
            \asegura{\paraTodo[unalinea]{i}{\ent}{
                ((0 \leq i < |transacciones|) \yLuego \neg (perteneceUsuario(B_0.usuarios, transacciones[i].id\_vendedor))) \\
                \implicaLuego (perteneceUsuario(b.usuarios, transacciones[i].id\_vendedor))
                }}
            ...
            \asegura{\paraTodo[unalinea]{i}{\ent}{(0 \leq i < |b.usuarios|) \implicaLuego \bbig( \\
                ...
                \big((\neg (perteneceUsuario(B_0.usuarios, b.usuarios[i].id))) \land (vendedorPerteneceATransaccion(b.usuarios[i], transacciones)) \implicaLuego
                    (b.usuarios[i].monedas = montoRecibido(transacciones, b.usuarios[i].id) - montoGastado(transacciones, b.usuario[i].id))\big) \\
                \qquad \oLuego \\
        ...
        \aux{montoRecibido}{transacciones: \TLista{transaccion}, id\_u: \ent, idx: \ent}{\ent}{
            \\ \qquad\quad \left(\sum_{k=0}^{idx} ifThenElse(transacciones[k].id\_vendedor = id\_u,\ transacciones[j].monto,\ 0)\right)}
        \vspace{0.5em}

        \aux{montoGastado}{transacciones: \TLista{transaccion}, id\_u: \ent, idx: \ent}{\ent}{
            \\ \qquad\quad \left(\sum_{k=0}^{idx} ifThenElse(transacciones[k].id\_comprador = id\_u,\ transacciones[j].monto,\ 0)\right)}
        \vspace{0.5em}
